...
export default function IdeaDetail({ idea, onClose, onEdit, onDelete }: IdeaDetailProps) {
  ...
  const handleDelete = () => {
    if (confirm('이 아이디어를 삭제할까요? 복구할 수 없습니다.')) {
      onDelete(idea.id);
    }
  };
  ...
  return (
    ...
    >
      {/* Action buttons — same grid as CardDetails */}
      <div className="grid grid-cols-4 gap-2 mb-6">
        {/* Tag badge large */}
        <div className="col-span-2 flex flex-col items-center justify-center p-3 rounded-2xl border border-neutral-800 bg-neutral-900 select-none gap-1">
      ...
      {/* Body content area — mirrors card info panel */}
      <div className="bg-neutral-900 border border-neutral-800 rounded-3xl p-5 mb-6 flex-1">
        <div className="pb-4 border-b border-neutral-800">
          <span className="text-[10px] font-bold tracking-widest text-neutral-400 uppercase">아이디어 내용</span>
          <p className="mt-3 text-[14px] leading-relaxed text-neutral-200 font-normal whitespace-pre-wrap break-words">
        ...
        <div className="flex justify-between items-center mt-4">
          ...
          <span className="text-[10px] font-mono tracking-widest font-black text-neutral-500">
            {idea.date}
          </span>
        </div>
      </div>

      {/* Delete — bottom danger zone */}
      <div className="mb-6">
        <button
          onClick={handleDelete}
          className="w-full flex items-center justify-center gap-2 p-3.5 rounded-2xl border border-rose-500/20 bg-rose-500/5 hover:bg-rose-500/10 active:scale-95 transition text-rose-400 select-none"
        >
